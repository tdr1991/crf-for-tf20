...
class CRF(Layer):
    ...
    def loss(self):
        def crf_nll(y_true, y_pred):
            ...
            nloglik = crf.get_negative_log_likelihood(y_true, y_pred, mask)
    ...
    def accuracy(self):
        def crf_viterbi_accuracy(y_true, y_pred):
            ...
            mask = K.any(K.not_equal(y_pred, 0), -1)
            y_pred = crf.viterbi_decoding(y_pred, mask)
        ...
        def crf_marginal_accuracy(y_true, y_pred):
            ...
            mask = K.any(K.not_equal(y_pred, 0), -1)
            y_pred = crf.get_marginal_prob(y_pred, mask)
    ...
    def get_negative_log_likelihood(self, y_true, input_energy, mask):
        ...
        logZ = self.get_log_normalization_constant(input_energy, mask,
                                                   input_length=K.int_shape(input_energy)[1])
        nloglik = logZ + energy
        if mask is not None:
            nloglik = nloglik / K.sum(K.cast(mask, K.floatx()), 1)
        else:
            nloglik = nloglik / K.cast(K.shape(input_energy)[1], K.floatx())
        return nloglik
    ...
    def viterbi_decoding(self, input_energy, mask=None):
        ...
        _, best_paths, _ = K.rnn(find_path, argmin_tables, initial_best_idx,
                                 input_length=K.int_shape(input_energy)[1], unroll=self.unroll)
